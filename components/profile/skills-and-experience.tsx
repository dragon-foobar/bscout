...
export default function SkillsAndExperience({
      ...
      <>
        <TextareaAutosize
          ...
          onInput={(e) => {
            setData({
              ...data,
              skillsAndExperience: (e.target as HTMLTextAreaElement).value,
            });
          }}
          ...
          value={data.skillsAndExperience}
        />
        <div className="flex justify-end w-full max-w-2xl">
          <p className="text-gray-400  text-sm">
            {data.skillsAndExperience.length}/500
          </p>
        </div>
      </>
    );
  ...
  return (
    <article className="mt-3 max-w-2xl text-sm tracking-wider leading-6 text-white  prose prose-headings:text-white prose-a:text-white">
      <MDXRemote {...data.skillsAndExperienceMdx} />
    </article>
  );
}
